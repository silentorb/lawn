...
export function process(method:string, request:Ground.External_Query_Source, user:Vineyard.IUser, vineyard:Vineyard, socket, callback):Promise {
  var fortress = vineyard.bulbs.fortress
  var action:any = method == 'query' ? query : update
  return prepare_fortress(fortress, user)
...
function query(request:Ground.External_Query_Source, user:Vineyard.IUser, lawn):Promise {
...
function run_query(query:Ground.Query_Builder, user:Vineyard.IUser, vineyard:Vineyard, request:Ground.External_Query_Source):Promise {
...
export function update(request:common.Update_Request, user, ground:Ground.Core, vineyard:Vineyard):Promise {
  if (vineyard.bulbs['lawn'].config.require_version === true && !request.version)
    throw new HttpError('The request must have a version property.', 400, 'version-required')
